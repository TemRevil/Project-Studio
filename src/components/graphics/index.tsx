...
export const AnimatedArrow = ({ fromX, fromY, toX, toY, startFrame = 0, color = COLORS.sky, strokeWidth = 3 }: {
  ...
  return (
    <svg style={{ position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none" }} width="100%" height="100%">
      ...
      {progress > 0.05 && (
        <line x1={fromX} y1={fromY} x2={endX} y2={endY} stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" markerEnd={progress > 0.9 ? "url(#ah)" : undefined} />
      )}
    </svg>
  );
};
...
export const DrawingLine = ({ x1, y1, x2, y2, startFrame = 0, color = COLORS.mauve, strokeWidth = 2, dashed = false }: {
  ...
  return (
    <svg style={{ position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none" }} width="100%" height="100%">
      {progress > 0.02 && (
        <line x1={x1} y1={y1} x2={x1 + (x2 - x1) * progress} y2={y1 + (y2 - y1) * progress} stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" strokeDasharray={dashed ? "8 6" : undefined} opacity={0.8} />
      )}
    </svg>
  );
};
...
export const HandwritingPath = ({ d, pathLength, startFrame = 0, color = COLORS.navy, strokeWidth = 3, duration = 60 }: {
  ...
  return (
    <svg style={{ position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none" }} width="100%" height="100%">
      <path d={d} fill="none" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" strokeDasharray={pathLength} strokeDashoffset={pathLength * (1 - eased)} />
    </svg>
  );
};
